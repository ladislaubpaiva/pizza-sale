...
pizzaJson.map((item, index) => {
  ...
  pizzaItem.querySelector('a').addEventListener('click', (event) => {
    ...
    selector('.pizzaInfo--actualPrice').innerText = `$ ${pizzaJson[
      key
    ].price[1].toFixed(2)}`;
    ...
    selectorAll('.pizzaInfo--size').forEach((size, index) => {
      ...
      size.addEventListener('click', () => {
        ...
        switch (Number(sizeSelect)) {
          case 0:
            selector('.pizzaInfo--actualPrice').innerText = `$ ${pizzaJson[key].price[0].toFixed(2)}`;
            break;
          case 1:
            selector('.pizzaInfo--actualPrice').innerText = `$ ${pizzaJson[key].price[1].toFixed(2)}`;
            break;
          case 2:
            selector('.pizzaInfo--actualPrice').innerText = `$ ${pizzaJson[key].price[2].toFixed(2)}`;
            break;
        }
      });
    });
